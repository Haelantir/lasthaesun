...
after a seed — a seed publishes content without deploying, so nothing else
warms the new rows — or from the Actions tab.

The workflow installs with `npm install`, not `npm ci`: the committed lock file
cannot satisfy `npm ci` on Linux under npm 10 (it was generated by npm 11 on
Node 24, which nests tsx's esbuild elsewhere). The runner is pinned to Node 24
for the same reason. Regenerating the lock would change what the next Vercel
build resolves, so it was left alone.
...
**A deployment empties the cache, so every push needs a warm — not just the
pushes that add content.** Measured directly rather than inferred from traffic:
all 396 urls were warmed to `HIT` (median 16ms), then a code-only push went out,
and the workflow that ran on that deployment reported
`MISS 388 / PRERENDER 5 / HIT 3`. No content had changed. The build id had.

That is why the workflow triggers on `deployment_status` rather than after a
seed, and why it is not optional: a deploy without it hands Googlebot several
hundred cold renders, which is the signal that taught it to crawl less to begin
with.
